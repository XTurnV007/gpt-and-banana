...
import { createDefaultNanoBananaProfile, DEFAULT_SETTINGS, normalizeSettings } from './apiProfiles'
...
describe('callImageApi', () => {
  ...
  it('uses the compatible generations request when no reference image is uploaded', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      created: 1,
      data: { b64_json: 'aW1hZ2U=' },
      output_format: 'jpeg',
    ...
    const result = await callImageApi({
      ...
      params: { ...DEFAULT_PARAMS, size: '1536x1024', quality: 'high', output_format: 'jpeg' },
      inputImageDataUrls: [],
    })

    expect(fetchMock).toHaveBeenCalledTimes(1)
    const [url, init] = fetchMock.mock.calls[0]
    expect(url).toBe('https://api.cursorai.art/v1/images/generations')
    ...
    expect(result).toMatchObject({
      ...
      actualParams: { output_format: 'jpeg', quality: 'high', size: '1536x1024' },
    })
  })

  it('automatically uses the compatible edits request when reference images are uploaded', async () => {
    ...
    const result = await callImageApi({
      ...
      prompt: 'merge them into one image',
      params: { ...DEFAULT_PARAMS, size: '1024x1536', quality: 'medium', n: 1 },
      inputImageDataUrls: [
    ...
    const apiCall = fetchMock.mock.calls.find(([input]) => !String(input).startsWith('data:'))
    expect(apiCall).toBeDefined()
    const [url, init] = apiCall!
    expect(url).toBe('https://api.cursorai.art/v1/images/edits')
    expect((init as RequestInit).headers).toMatchObject({
      Accept: 'application/json',
      Authorization: 'Bearer test-key',
    })
    ...
    expect(formData.has('output_format')).toBe(false)
    expect(result).toMatchObject({
      images: ['data:image/png;base64,ZWRpdGVk'],
      actualParams: { output_format: 'png', quality: 'medium', size: '1024x1536' },
    })
  })

  it('uses the Nano Banana generateContent endpoint for generation and reference-image editing', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      candidates: [{
        content: {
          parts: [{
    ...
    const profile = createDefaultNanoBananaProfile({
      id: 'nano-banana-test',
      apiKey: 'banana key',
    })
    ...
    const result = await callImageApi({
      settings,
      prompt: '把它画成漫画',
      params: { ...DEFAULT_PARAMS, banana_aspect_ratio: '3:2', banana_quality: 'high' },
      inputImageDataUrls: ['data:image/jpeg;base64,aW1hZ2U='],
    })

    expect(fetchMock).toHaveBeenCalledTimes(1)
    const [url, init] = fetchMock.mock.calls[0]
    expect(url).toBe('https://api.cursorai.art/v1beta/models/gemini-3.1-flash-image-preview:generateContent?key=banana+key')
    ...
    expect(result).toMatchObject({
      images: ['data:image/png;base64,YmFuYW5h'],
      actualParams: { banana_image_size: '2K', banana_aspect_ratio: '3:2', banana_quality: 'high', output_format: 'png', n: 1 },
    })
  })

  it('uses Nano Banana native default image config', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      candidates: [{ content: { parts: [{ inline_data: { mime_type: 'image/png', data: 'YmFuYW5h' } }] } }],
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    }))
    ...
    const [, init] = fetchMock.mock.calls[0]
    expect(JSON.parse(String((init as RequestInit).body)).generationConfig.imageConfig).toEqual({
      aspectRatio: '1:1',
      imageSize: '2K',
      quality: 'standard',
    })
  ...
  it('routes Nano Banana through the locked deployment API proxy', async () => {
    ...
    const profile = createDefaultNanoBananaProfile({ id: 'nano-proxy', apiKey: 'test-key' })
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      '/api-proxy/v1beta/models/gemini-3.1-flash-image-preview:generateContent?key=test-key',
    )
  })
