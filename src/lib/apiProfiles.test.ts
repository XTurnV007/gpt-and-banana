...
\`\`\``)
  ...
  it('keeps provider order usable when custom providers are added after manual sorting', () => {
    ...
    expect(settings.providerOrder).toEqual(['fal', 'openai', 'nano-banana', 'custom-alpha', 'custom-beta'])
  })
  ...
  it('keeps non-OpenAI providers in Images API mode when switching providers', () => {
    ...
    const nanoBananaProfile = switchApiProfileProvider(openaiProfile, 'nano-banana')
    ...
    expect(nanoBananaProfile).toMatchObject({
  ...
  it('normalizes Nano Banana as a built-in provider', () => {
    const profile = createDefaultNanoBananaProfile({ id: 'nano-banana-test', apiKey: 'test-key' })
