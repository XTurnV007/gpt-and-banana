...
function normalizeImageApiPayload(value: unknown): ImageApiResponse {
  ...
  if (value && typeof value === 'object') {
    ...
    if (Array.isArray(data)) return payload as unknown as ImageApiResponse
    if (data && typeof data === 'object') {
      return { ...payload, data: [data as ImageResponseItem] } as ImageApiResponse
    }
    if (typeof payload.b64_json === 'string' || typeof payload.url === 'string') {
      return { ...payload, data: [payload as ImageResponseItem] } as ImageApiResponse
    }
    return { ...payload, data: [] } as ImageApiResponse
  }
  return { data: [] }
}
...
async function parseImagesApiResponse(rawPayload: unknown, mime: string, signal?: AbortSignal): Promise<CallApiResult> {
  const payload = normalizeImageApiPayload(rawPayload)
  const data = payload.data
  ...
  try {
    for (const item of data) {
      const responseFormat = item.output_format ?? item.format ?? payload.output_format ?? payload.format
      const itemMime = typeof responseFormat === 'string' ? MIME_MAP[responseFormat] ?? mime : mime
...
async function callImagesApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    ...
    if (isEdit) {
      if (inputImageDataUrls.length > MAX_EDIT_IMAGE_COUNT) {
        throw new Error(`参考图数量超过上限：最多 ${MAX_EDIT_IMAGE_COUNT} 张`)
      }
      ...
      for (let i = 0; i < inputImageDataUrls.length; i++) {
        ...
        assertImageEditFileSize(`参考图 ${i + 1}`, blob.size)
      ...
      for (let i = 0; i < imageBlobs.length; i++) {
        ...
        formData.append('file', blob, `input-${i + 1}.${ext}`)
      }
