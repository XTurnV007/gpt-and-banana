...
const OPENAI_DEFAULT_BASE_URL = 'https://api.cursorai.art/v1'
...
export const DEFAULT_NANO_BANANA_BASE_URL = 'https://api.cursorai.art'
export const DEFAULT_NANO_BANANA_MODEL = 'gemini-3.1-flash-image-preview'
export const DEFAULT_OPENAI_PROFILE_ID = 'default-openai'
export const DEFAULT_NANO_BANANA_PROFILE_ID = 'default-nano-banana'
export const DEFAULT_API_TIMEOUT = 600

const BUILT_IN_PROVIDER_IDS = new Set<ApiProvider>(['openai', 'fal', 'nano-banana'])
...
function normalizeProviderOrder(value: unknown, customProviders: CustomProviderDefinition[]): string[] | undefined {
  ...
  const providerIds = ['openai', 'nano-banana', 'fal', ...customProviders.map((provider) => provider.id)]
...
export function createDefaultNanoBananaProfile(overrides: Partial<ApiProfile> = {}): ApiProfile {
  return {
    id: `nano-banana-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`,
    name: 'Nano Banana',
    provider: 'nano-banana',
    baseUrl: DEFAULT_NANO_BANANA_BASE_URL,
    apiKey: '',
...
export function createBuiltInDefaultProfiles(): ApiProfile[] {
  return [
    createDefaultOpenAIProfile({
      id: DEFAULT_OPENAI_PROFILE_ID,
      name: DEFAULT_API_URL_PATCH?.name ?? 'gpt-image-2',
    }),
    createDefaultNanoBananaProfile({
      id: DEFAULT_NANO_BANANA_PROFILE_ID,
      name: 'nano-banana-2',
      baseUrl: DEFAULT_NANO_BANANA_BASE_URL,
      model: DEFAULT_NANO_BANANA_MODEL,
    }),
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  ...
  if (provider === 'nano-banana') {
    return {
      ...profile,
      provider,
      baseUrl: savedDraft?.baseUrl ?? DEFAULT_NANO_BANANA_BASE_URL,
      model: savedDraft?.model ?? DEFAULT_NANO_BANANA_MODEL,
      apiMode: 'images',
      codexCli: false,
      apiProxy: false,
  ...
  if (customProvider) {
    const shouldUseOpenAIDefaults = profile.provider === 'fal' || profile.provider === 'nano-banana'
    return {
      ...profile,
...
function normalizeProviderDraft(input: unknown, provider: ApiProvider, customProviderIds: Set<string>): ApiProfileProviderDraft {
  ...
  const knownProvider = provider === 'fal' || provider === 'nano-banana' || provider === 'openai' || customProviderIds.has(provider)
  ...
  return {
    baseUrl: provider === 'fal'
      ...
      : provider === 'nano-banana'
        ? baseUrl?.trim().replace(/\/+$/, '') || DEFAULT_NANO_BANANA_BASE_URL
        : baseUrl,
    model,
    apiMode,
...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  ...
  const provider: ApiProvider = rawProvider === 'fal' || rawProvider === 'nano-banana' || customProviderIds.has(rawProvider) ? rawProvider : 'openai'
  ...
  const defaults = provider === 'fal'
    ...
    : provider === 'nano-banana'
      ? createDefaultNanoBananaProfile(fallback)
  ...
  return {
    ...
    baseUrl: provider === 'fal'
      ...
      : provider === 'nano-banana'
        ? rawBaseUrl.trim().replace(/\/+$/, '') || DEFAULT_NANO_BANANA_BASE_URL
        : rawBaseUrl,
...
function isDefaultOpenAIProfile(profile: ApiProfile): boolean {
  return profile.id === DEFAULT_OPENAI_PROFILE_ID &&
    profile.name === (DEFAULT_API_URL_PATCH?.name ?? 'gpt-image-2') &&
    profile.provider === 'openai' &&
    profile.baseUrl === DEFAULT_BASE_URL &&
    profile.apiKey === (DEFAULT_API_URL_PATCH?.apiKey ?? '') &&
    profile.model === (DEFAULT_API_URL_PATCH?.model ?? DEFAULT_IMAGES_MODEL) &&
    profile.timeout === DEFAULT_API_TIMEOUT &&
    profile.apiMode === (DEFAULT_API_URL_PATCH?.apiMode ?? 'images') &&
    profile.codexCli === (DEFAULT_API_URL_PATCH?.codexCli ?? false) &&
    profile.apiProxy === DEFAULT_OPENAI_API_PROXY &&
    profile.streamImages === (DEFAULT_API_URL_PATCH?.streamImages ?? getDefaultStreamImages('openai', DEFAULT_API_URL_PATCH?.apiMode ?? 'images')) &&
    profile.streamPartialImages === (DEFAULT_API_URL_PATCH?.streamPartialImages ?? DEFAULT_STREAM_PARTIAL_IMAGES)
}

function isDefaultNanoBananaProfile(profile: ApiProfile): boolean {
  return profile.id === DEFAULT_NANO_BANANA_PROFILE_ID &&
    profile.name === 'nano-banana-2' &&
    profile.provider === 'nano-banana' &&
    profile.baseUrl === DEFAULT_NANO_BANANA_BASE_URL &&
    profile.apiKey === '' &&
    profile.model === DEFAULT_NANO_BANANA_MODEL &&
    profile.timeout === DEFAULT_API_TIMEOUT &&
...
function hasOnlyDefaultProfiles(settings: AppSettings): boolean {
  return settings.customProviders.length === 0 &&
    ...
    isDefaultNanoBananaProfile(settings.profiles[1])
}
...
export const DEFAULT_SETTINGS: AppSettings = normalizeSettings({
  ...
  profiles: createBuiltInDefaultProfiles(),
