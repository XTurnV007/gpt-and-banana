...
import { createDefaultFalProfile, createDefaultOpenAIProfile, DEFAULT_NANO_BANANA_PROFILE_ID, DEFAULT_RESPONSES_MODEL, DEFAULT_SETTINGS, normalizeSettings } from './lib/apiProfiles'
...
describe('agent conversation persistence', () => {
  ...
  it('adds the Nano Banana built-in profile once when upgrading persisted settings', () => {
    const legacyProfile = createDefaultOpenAIProfile({ name: '默认', baseUrl: 'https://api.openai.com/v1' })
    const legacyState = {
      settings: normalizeSettings({ profiles: [legacyProfile], activeProfileId: legacyProfile.id }),
    }

    const migrated = migratePersistedState(legacyState, 2) as { settings: AppSettings }
    ...
    expect(migrated.settings.profiles[1]).toMatchObject({
      id: DEFAULT_NANO_BANANA_PROFILE_ID,
      name: 'nano-banana-2',
      baseUrl: 'https://api.cursorai.art',
      provider: 'nano-banana',
    })

    const afterManualDelete = migratePersistedState(legacyState, 3) as { settings: AppSettings }
    expect(afterManualDelete.settings.profiles).toHaveLength(1)
  })
})
