...
import { createBuiltInDefaultProfiles, DEFAULT_NANO_BANANA_PROFILE_ID, DEFAULT_OPENAI_PROFILE_ID, DEFAULT_SETTINGS, getActiveApiProfile, getAgentImageApiProfile, getAgentTextApiProfile, getCustomProviderDefinition, mergeImportedSettings, normalizeSettings, validateApiProfile } from './lib/apiProfiles'
...
export function migratePersistedState(persistedState: unknown, persistedVersion = 2): unknown {
  if (!isRecord(persistedState)) return persistedState
  const shouldAddBuiltInProfiles = persistedVersion < 3
  const settings = shouldAddBuiltInProfiles && isRecord(persistedState.settings)
    ? (() => {
        const normalized = normalizeSettings(persistedState.settings)
        const profiles = normalized.profiles.map((profile) => {
          if (profile.id !== DEFAULT_OPENAI_PROFILE_ID) return profile
          return {
            ...profile,
            name: profile.name === '默认' ? 'gpt-image-2' : profile.name,
            baseUrl: !profile.baseUrl || profile.baseUrl === 'https://api.openai.com/v1'
              ? 'https://api.cursorai.art/v1'
              : profile.baseUrl,
          }
        })
        if (!profiles.some((profile) => profile.id === DEFAULT_NANO_BANANA_PROFILE_ID || profile.provider === 'nano-banana')) {
          profiles.push(createBuiltInDefaultProfiles()[1])
        }
...
function getCustomRecoveryProfile(settings: AppSettings, task: TaskRecord) {
  ...
  if (!provider || provider === 'openai' || provider === 'fal' || provider === 'nano-banana') return null
  const taskProfile = getTaskApiProfile(settings, task)
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    ...
    for (let index = 0; index < plan.length; index++) {
      ...
      a.href = url
      a.download = `gpt-and-banana-backup_${formatExportFileTime(new Date(exportedAt))}${suffix}.zip`
