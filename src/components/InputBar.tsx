...
import InputParamsPanel, { type ImageSizeMode } from './input/inputParamsPanel'

const IMAGE_SIZE_TIERS: SizeTier[] = ['1K', '2K', '4K']
const IMAGE_SIZE_RATIOS = ['1:1', '3:2', '2:3', '16:9', '9:16', '4:3', '3:4', '21:9']
const NANO_BANANA_SIZE_TIERS: SizeTier[] = ['2K']
const NANO_BANANA_SIZE_RATIOS = ['1:1', '4:3', '3:4', '16:9', '9:16', '2:3', '3:2']
const NANO_BANANA_SIZE_CONFIG_PATTERN = /^(2K):(.+)$/i

function createNanoBananaSizeValue(tier: SizeTier, ratio: string) {
  return `${tier}:${ratio}`
}
...
function findImageSizePreset(size: string) {
...
function inferImageSizeControls(size: string): {
  mode: ImageSizeMode
  tier: SizeTier
  ratio: string
  width: string
  ...
  const nanoBananaSizeConfig = size.match(NANO_BANANA_SIZE_CONFIG_PATTERN)
  if (nanoBananaSizeConfig) {
    return { mode: 'ratio', tier: nanoBananaSizeConfig[1].toUpperCase() as SizeTier, ratio: nanoBananaSizeConfig[2], width: '1024', height: '1024' }
  }

  if (!size || size === 'auto') {
    return { mode: 'auto', tier: '1K', ratio: '1:1', width: '1024', height: '1024' }
  }

  const preset = findImageSizePreset(size)
  const dimensions = parseImageSizeValue(normalizeImageSize(size))
  return {
    mode: preset ? 'ratio' : 'resolution',
    tier: preset?.tier ?? '1K',
...
export default function InputBar() {
  ...
  const initialSizeControlsRef = useRef(inferImageSizeControls(params.size))
  const [sizeMode, setSizeMode] = useState<ImageSizeMode>(initialSizeControlsRef.current.mode)
  const [sizeTier, setSizeTier] = useState<SizeTier>(initialSizeControlsRef.current.tier)
  const [sizeRatio, setSizeRatio] = useState(initialSizeControlsRef.current.ratio)
  const [customSizeWidth, setCustomSizeWidth] = useState(initialSizeControlsRef.current.width)
  const [customSizeHeight, setCustomSizeHeight] = useState(initialSizeControlsRef.current.height)
  const lastSizeControlValueRef = useRef<string | null>(null)
  ...
  const handleApiProfileChange = useCallback((profileId: string) => {
    if (!settings.profiles.some((profile) => profile.id === profileId)) return

    setReusedTaskApiProfile(null)

    if (appMode !== 'agent' || settings.agentApiConfigMode === 'off') {
      setSettings({ activeProfileId: profileId })
      return
    }

    if (settings.agentApiConfigMode === 'hybrid') {
      setSettings({ agentImageProfileId: profileId })
      return
    }

    setSettings({ agentTextProfileId: profileId })
  }, [appMode, setReusedTaskApiProfile, setSettings, settings.agentApiConfigMode, settings.profiles])
  ...
  const apiProfileOptions = useMemo(() => settings.profiles.map((profile) => ({
    value: profile.id,
    label: profile.name === profile.model ? profile.name : `${profile.name} · ${profile.model}`,
  })), [settings.profiles])
  const activeSizeTierOptions = activeProvider === 'nano-banana' ? NANO_BANANA_SIZE_TIERS : IMAGE_SIZE_TIERS
  const activeSizeRatioOptions = activeProvider === 'nano-banana' ? NANO_BANANA_SIZE_RATIOS : IMAGE_SIZE_RATIOS
  const allowResolutionSizeMode = activeProvider !== 'nano-banana'
  const agentAutoImageCount = appMode === 'agent'
  const outputImageLimit = getOutputImageLimitForSettings(effectiveSettings)
  const isFalTextToImage = isFalProvider && inputImages.length === 0
  const allowAutoSizeMode = !isFalTextToImage
  ...
  const applySizeControlValue = useCallback((size: string) => {
    lastSizeControlValueRef.current = size
    setParams({ size })
  }, [setParams])

  const calculateActiveImageSize = useCallback((tier: SizeTier, ratio: string) => (
    activeProvider === 'nano-banana' ? createNanoBananaSizeValue(tier, ratio) : calculateImageSize(tier, ratio)
  ), [activeProvider])

  const applyNanoBananaImageConfig = useCallback((tier: SizeTier, ratio: string, quality = params.banana_quality) => {
    const nextTier = '2K'
    const nextRatio = NANO_BANANA_SIZE_RATIOS.includes(ratio) ? ratio as typeof params.banana_aspect_ratio : '1:1'
    lastSizeControlValueRef.current = createNanoBananaSizeValue(nextTier, nextRatio)
    setParams({
      size: createNanoBananaSizeValue(nextTier, nextRatio),
      banana_image_size: nextTier,
      banana_aspect_ratio: nextRatio,
      banana_quality: quality,
    })
  }, [params.banana_quality, setParams])

  const applyCustomSize = useCallback((width: string, height: string) => {
    const parsedWidth = Number(width)
    const parsedHeight = Number(height)
    if (!Number.isFinite(parsedWidth) || !Number.isFinite(parsedHeight) || parsedWidth <= 0 || parsedHeight <= 0) return
    const normalizedSize = normalizeImageSize(`${Math.round(parsedWidth)}x${Math.round(parsedHeight)}`)
    const normalizedDimensions = parseImageSizeValue(normalizedSize)
    if (!normalizedDimensions) return
    setCustomSizeWidth(normalizedDimensions.width)
    setCustomSizeHeight(normalizedDimensions.height)
    applySizeControlValue(normalizedSize)
  }, [applySizeControlValue])

  const handleSizeModeChange = useCallback((mode: ImageSizeMode) => {
    const nextMode = (mode === 'auto' && !allowAutoSizeMode) || (mode === 'resolution' && !allowResolutionSizeMode) ? 'ratio' : mode
    setSizeMode(nextMode)
    if (nextMode === 'auto') {
      applySizeControlValue('auto')
      return
    }
    if (nextMode === 'ratio') {
      const size = calculateActiveImageSize(sizeTier, sizeRatio)
      if (size) applySizeControlValue(size)
      return
    }
    applyCustomSize(customSizeWidth, customSizeHeight)
  }, [allowAutoSizeMode, allowResolutionSizeMode, applyCustomSize, applySizeControlValue, calculateActiveImageSize, customSizeHeight, customSizeWidth, sizeRatio, sizeTier])

  const handleSizeTierChange = useCallback((value: string) => {
    const tier = activeSizeTierOptions.includes(value as SizeTier) ? value as SizeTier : activeSizeTierOptions[0]
    setSizeTier(tier)
    if (activeProvider === 'nano-banana') {
      applyNanoBananaImageConfig(tier, sizeRatio)
      return
    }
    const size = calculateActiveImageSize(tier, sizeRatio)
    if (size) applySizeControlValue(size)
  }, [activeProvider, activeSizeTierOptions, applyNanoBananaImageConfig, applySizeControlValue, calculateActiveImageSize, sizeRatio])

  const handleSizeRatioChange = useCallback((ratio: string) => {
    const nextRatio = activeSizeRatioOptions.includes(ratio) ? ratio : activeSizeRatioOptions[0]
    setSizeRatio(nextRatio)
    if (activeProvider === 'nano-banana') {
      applyNanoBananaImageConfig(sizeTier, nextRatio)
      return
    }
    const size = calculateActiveImageSize(sizeTier, nextRatio)
    if (size) applySizeControlValue(size)
  }, [activeProvider, activeSizeRatioOptions, applyNanoBananaImageConfig, applySizeControlValue, calculateActiveImageSize, sizeTier])

  const handleBananaQualityChange = useCallback((quality: 'standard' | 'high') => {
    applyNanoBananaImageConfig(sizeTier, sizeRatio, quality)
  }, [applyNanoBananaImageConfig, sizeRatio, sizeTier])

  useEffect(() => {
    if (activeProvider !== 'nano-banana') return

    const nextTier: SizeTier = params.banana_image_size || '2K'
    const nextRatio = activeSizeRatioOptions.includes(params.banana_aspect_ratio)
      ? params.banana_aspect_ratio
      : '1:1'
    const nextSize = createNanoBananaSizeValue(nextTier, nextRatio)

    if (sizeMode !== 'ratio') setSizeMode('ratio')
    if (sizeTier !== nextTier) setSizeTier(nextTier)
    if (sizeRatio !== nextRatio) setSizeRatio(nextRatio)
    if (
      params.size !== nextSize ||
      params.banana_image_size !== nextTier ||
      params.banana_aspect_ratio !== nextRatio ||
      (params.banana_quality !== 'standard' && params.banana_quality !== 'high')
    ) {
      lastSizeControlValueRef.current = nextSize
      setParams({
        size: nextSize,
        banana_image_size: nextTier,
        banana_aspect_ratio: nextRatio,
        banana_quality: params.banana_quality === 'high' ? 'high' : 'standard',
      })
    }
  }, [
    activeProvider,
    activeSizeRatioOptions,
    params.banana_aspect_ratio,
    params.banana_image_size,
    params.banana_quality,
    params.size,
    setParams,
    sizeMode,
    sizeRatio,
  ...
  useEffect(() => {
    ...
    let nextMode = sizeMode
    let nextTier = activeSizeTierOptions.includes(sizeTier) ? sizeTier : activeSizeTierOptions[0]
    let nextRatio = activeSizeRatioOptions.includes(sizeRatio) ? sizeRatio : activeSizeRatioOptions[0]

    if (!allowAutoSizeMode && nextMode === 'auto') nextMode = 'ratio'
    if (!allowResolutionSizeMode && nextMode === 'resolution') nextMode = 'ratio'

    if (nextTier !== sizeTier) setSizeTier(nextTier)
    if (nextRatio !== sizeRatio) setSizeRatio(nextRatio)
    if (nextMode !== sizeMode) setSizeMode(nextMode)

    if (nextMode === 'auto') {
      if (params.size !== 'auto') applySizeControlValue('auto')
      return
    }

    if (nextMode === 'ratio') {
      const size = calculateActiveImageSize(nextTier, nextRatio)
      if (size && params.size !== size) applySizeControlValue(size)
    }
  }, [
    activeSizeRatioOptions,
    activeSizeTierOptions,
    allowAutoSizeMode,
    allowResolutionSizeMode,
    applySizeControlValue,
    calculateActiveImageSize,
    params.size,
    sizeMode,
    sizeRatio,
    sizeTier,
  ...
  const handleCustomSizeWidthChange = useCallback((value: string) => {
    setCustomSizeWidth(value)
  }, [])

  const handleCustomSizeHeightChange = useCallback((value: string) => {
    setCustomSizeHeight(value)
  }, [])

  const commitCustomSize = useCallback(() => {
    applyCustomSize(customSizeWidth, customSizeHeight)
  }, [applyCustomSize, customSizeHeight, customSizeWidth])

  useEffect(() => {
    if (lastSizeControlValueRef.current === params.size) {
      lastSizeControlValueRef.current = null
      return
    }

    const controls = inferImageSizeControls(params.size)
    setSizeMode(controls.mode)
    setSizeTier(controls.tier)
    setSizeRatio(controls.ratio)
    setCustomSizeWidth(controls.width)
    setCustomSizeHeight(controls.height)
  }, [params.size])
  ...
  }, [params.moderation, params.output_compression, params.output_format, params.quality, params.transparent_output, setParams])
  ...
  const renderParams = (layout: 'desktop' | 'mobile') => (
    <InputParamsPanel
      layout={layout}
      apiProfileId={activeProfile.id}
      apiProfileOptions={apiProfileOptions}
      onApiProfileChange={handleApiProfileChange}
      isNanoBanana={activeProvider === 'nano-banana'}
      bananaQuality={params.banana_quality || 'standard'}
      onBananaQualityChange={handleBananaQualityChange}
      sizeMode={sizeMode}
      onSizeModeChange={handleSizeModeChange}
      allowAuto={allowAutoSizeMode}
      allowResolution={allowResolutionSizeMode}
      sizeTier={sizeTier}
      onSizeTierChange={handleSizeTierChange}
      sizeTierOptions={activeSizeTierOptions}
      sizeRatio={sizeRatio}
      onSizeRatioChange={handleSizeRatioChange}
      sizeRatioOptions={activeSizeRatioOptions}
      customWidth={customSizeWidth}
      customHeight={customSizeHeight}
      onCustomWidthChange={handleCustomSizeWidthChange}
      onCustomHeightChange={handleCustomSizeHeightChange}
      onCustomSizeCommit={commitCustomSize}
